...
router.post('/request', async (req, res, next) => {
  ...
  try {
    // Rate limiting desactive : demandes OTP illimitees
    ...
    try {
      ...
      return res.status(500).json({ success: false, message: 'Erreur envoi email', code: 'EMAIL_ERROR', details: emailErr.message });
    }
  } catch (err) {
    if (err.msBeforeNext) {
      return res.status(429).json({ success: false, message: 'Trop de requetes, reessayez plus tard', code: 'RATE_LIMITED' });
    }
    next(err);
  }
});
...
router.post('/verify', async (req, res, next) => {
  ...
  try {
    ...
    if (!row) return res.status(400).json({ success: false, message: 'Code non trouve', code: 'CODE_NOT_FOUND' });
    if (row.attempts >= MAX_ATTEMPTS) return res.status(400).json({ success: false, message: 'Trop de tentatives', code: 'TOO_MANY_ATTEMPTS' });
    if (new Date(row.expires_at) < new Date()) return res.status(400).json({ success: false, message: 'Code expire', code: 'CODE_EXPIRED' });
    ...
    // Sessions peuvent rester en SQLite ou etre migrees Firestore si besoin
...
router.post('/refresh', async (req, res, next) => {
  ...
  try {
    ...
    if (!session) return res.status(400).json({ success: false, message: 'Session non trouvee', code: 'SESSION_NOT_FOUND' });
    if (new Date(session.expires_at) < new Date()) return res.status(400).json({ success: false, message: 'Session expiree', code: 'SESSION_EXPIRED' });
    ...
    if (!user) return res.status(400).json({ success: false, message: 'Utilisateur non trouve', code: 'USER_NOT_FOUND' });
    const access_token = jwt.sign({ user_id: user.id, email: user.email }, process.env.JWT_SECRET, { expiresIn: '15m' });
